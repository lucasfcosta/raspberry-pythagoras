...
tweetHandler.handle = function(tweet, callback) {
	...
	try {
		...
		if (e.asTweet) {
			responseText = this.createErrorResponse(tweet.user.screen_name, e.dict);
		} else {
			error = e;	
		}		
	}
...
tweetHandler.calculateResults = function(operations) {
	...
	operations.forEach((operation) => {
		...
		if (this.matchDivisionByZero(operation)) {
			let error = new Error('Division by zero');
			error.asTweet = true;
			error.dict = 'divisionByZeroErrors';
			throw error;
		}
...
tweetHandler.createResponse = function(username, results) {
	...
	if (finalResponse.length > 140) {
		finalResponse = this.createLengthResponse();
	}

	return finalResponse;
}

tweetHandler.createErrorResponse = function(username, dict) {
	let randomGreeting = Math.floor(Math.random() * (this.dictionary.greetings.length));
	let randomAnswerText = Math.floor(Math.random() * (this.dictionary[dict].length));

	let chosenAnswerText = this.dictionary[dict][randomAnswerText];	
	let chosenGreeting = this.dictionary.greetings[randomGreeting];
...
tweetHandler.createLengthResponse = function(username) {
	let randomErrorText = Math.floor(Math.random() * (this.dictionary.lengthErrors.length - 1)) + 1;
	return `@${username} ${this.dictionary.lengthErrors[randomErrorText]}`;
}
