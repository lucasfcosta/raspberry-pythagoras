...
tweetHandler.handle = function(tweet) {
	...
	return this.createResponse(tweet.user.name, results);
}
...
tweetHandler.createResponse = function(username, results) {
	...
	if (finalResponse.length > 140) {
		let randomErrorText = Math.floor(Math.random() * (this.dictionary.errors.length - 1)) + 1;
		finalResponse = `@${username} ${this.dictionary.errors[randomErrorText]}`;
	}
...
//var final = tweetHandler.handle({text: 'How much is 2+2*3 and sin(45)?', user: {name: 'randomBoy'}});
//console.log(final);
